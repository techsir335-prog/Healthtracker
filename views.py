...
def predict(request):
    ...
    try:
        ...
        # Rule-based fine-tune
        rule_score = compute_health_score(age, weight, height, gender, activity, total_cal, protein, carbs, fat, fiber)

        # Blend both scores
        final_score = round((ml_score * 0.5 + rule_score * 0.5), 1)

        # Lifespan curve
        ages, scores = generate_lifespan_curve(final_score, age)
        ...
        return JsonResponse({
            ...
            "lifespan_curve": {"ages": ages, "scores": scores},
